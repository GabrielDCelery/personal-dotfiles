  ...
  config = function()
    require('gitsigns').setup {
      ...
      on_attach = function(bufnr)
        ...
        map('n', '<leader>ggm', function()
          gitsigns.change_base(require('custom.git-default-branch').get(), true)
        end, { desc = 'Gitsigns diff against [m]ain/master' })
        ...
        end, { desc = 'Gitsigns diff against [b]ranch' })
        map('n', '<leader>ggr', function()
          gitsigns.reset_base(true)
        end, { desc = 'Gitsigns [r]eset diff base' })
    ...
    end, { desc = 'Gitsigns full buffer [B]lame (side panel, D = diff commit)' })
  end,
}
